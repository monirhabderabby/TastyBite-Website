...
const NavbarSearchModal = ({ open, setOpen }: Props) => {
  ...
  } else if (isError || !foods.success) {
    const msg =
      foods?.message ||
      "We couldn't fetch the food data at this time. Please try again later.";
    content = <ErrorState message={msg} />;
  } else if (foods.data?.length === 0) {
    content = (
      <div className="text-center py-40 text-primary-gray">
        <p className="text-lg font-semibold">No foods match your criteria.</p>
        <p>Try adjusting your search terms.</p>
      </div>
    );
  } else if (foods?.data?.length > 0) {
    content = (
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-x-2 gap-y-[30px] px-3 pb-10">
  ...
  return (
    <Modal open={open} setOpen={setOpen}>
      ...
      <ScrollArea className="h-[70vh] md:h-[450px] mt-5">{content}</ScrollArea>
    </Modal>
  );
};
