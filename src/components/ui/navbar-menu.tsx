    ...
    return (
        <div onMouseEnter={() => setActive(item)} className="relative">
            <motion.p
                ...
                className="cursor-pointer text-white hover:opacity-[0.9] dark:text-white flex items-center group"
            >
                {item}{" "}
                <ChevronDown className="w-5 ml-[2px] group-hover:rotate-180 duration-500" />
            </motion.p>
            {active !== null && (
                <motion.div
...
export const HoveredLink = ({ children, ...rest }: any) => {
    return (
        <Link
            {...rest}
            className="text-neutral-700 dark:text-neutral-200 hover:text-primary-orange text-base"
        >
            {children}
        </Link>
    );
